...
- 统计页支持 `周账 / 月账 / 年账` 切换。记账周固定按月内四段划分：1-7 号、8-14 号、15-21 号、22 号到月末。周账展示该周账单记录，月账展示按周次统计的支出折线图，年账展示按月份统计的支出折线图，并分别展示分类排行。
...
- 总结页主界面按 `年结 / 月结 / 周结` 三个切换展示统计内容，不再在下方显示可展开的年/月/周树；每个切换下方要直接展示对应总结内容。
...
- 弹窗标题不要重复。
- 总结弹窗支持上一个/下一个。
- 统计折线图横向滑动时要固定左侧 Y 轴数字，不能让字数或支出数随图一起滑走。
- 日记页搜索框需要压缩上下高度占比，横向宽度保持正常；搜索图标在右侧。
